...
public class PdfService {
    ...
    public Respuesta generarFichaPDF(Ficha ficha, Cliente cliente) {
        ...
        try {
            ...
            try (PDDocument doc = new PDDocument()) {
                ...
                try (InputStream regularStream = getClass().getResourceAsStream("/cr/ac/una/astroline/resource/fonts/GoogleSans-Regular.ttf"); InputStream boldStream = getClass().getResourceAsStream("/cr/ac/una/astroline/resource/fonts/GoogleSans-Bold.ttf"); InputStream italicStream = getClass().getResourceAsStream("/cr/ac/una/astroline/resource/fonts/GoogleSans-Italic.ttf")) {
    ...
    private float drawHeader(PDDocument doc, PDPageContentStream cs, Empresa emp,
        ...
        if (hasTelLine) {
            blockH += LINE_GAP + SZ_TEL * 0.72f;
        }
        if (hasDirLine) {
            blockH += LINE_GAP + SZ_DIR * 0.72f;
        }
    ...
    private float drawDateTime(PDPageContentStream cs, PDType0Font regular, PDType0Font bold, Ficha ficha, float y) throws IOException {
    ...
    private float drawTicketBox(PDPageContentStream cs, Ficha ficha, PDType0Font bold, PDType0Font regular, float y) throws IOException {
    ...
    private float drawClientSection(PDPageContentStream cs, Cliente cliente, PDType0Font bold, PDType0Font regular, float y) throws IOException {
    ...
    private float drawPreferencial(PDPageContentStream cs, Ficha ficha, PDType0Font bold, float y) throws IOException {
    ...
    private void drawFooter(PDPageContentStream cs, PDType0Font bold, PDType0Font italic) throws IOException {
    ...
    private void text(PDPageContentStream cs, PDType0Font font, float size, float x, float y, String t, Color c) throws IOException {
        if (t == null || t.isBlank()) {
            return;
        }
        cs.beginText();
